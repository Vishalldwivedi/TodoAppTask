...
http://<loadbalancer ExternalIP> 


AWS cloud controller manager will give u a network load balancer for this service with is a layer 4 load balancer

we can also cofigure ingress for better http https based host based path based routing it will safe the cost as in loadbalancer service type for every 

service a new NLB is created which it not good for cost 
